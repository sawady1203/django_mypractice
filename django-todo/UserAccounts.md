...
カスタムユーザーモデルに変更したため、views.pyも変更する。
...
```
としたが、一度Userオブジェクトでmigrationをしているため、migrateエラーが発生した。
__pycache__とmigrations/、db.sqlite3を消去してもう一度マイグレーションファイルを作成し、マイグレイトを実行した。
